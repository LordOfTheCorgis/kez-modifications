import { db } from "./db";
import type { PackRow, SubscriptionRow } from "./db";
...
export function hasActiveSubscription(userId: number): boolean {
  const sub = db
    ...
    .get(userId) as Pick<SubscriptionRow, "status"> | undefined;
  return sub?.status === "active";
}

/** All-access: active subscription, or ownership of any grants_all_access pack. */
